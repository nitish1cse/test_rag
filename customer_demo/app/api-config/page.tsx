...
export default function ApiConfigPage() {
  ...
  // Update endpoint preview when parameters change
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create API configuration');
      }

      router.push('/dashboard');
    } catch (error) {
      console.error('Error creating API config:', error);
      setError(error instanceof Error ? error.message : 'Failed to create API configuration');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-8 py-8">
        <Card>
          <CardHeader>
            <CardTitle>Configure New API</CardTitle>
          </CardHeader>
          <CardContent>
